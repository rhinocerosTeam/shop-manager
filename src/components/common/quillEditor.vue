...
<script>
  ...
  import UploadUtils from 'utils/uploadOSS'
    ...
    methods: {
      ...
      fileClick(type) {
        ...
        if (type == "image") {
          ...
          input.accept = "video/mp4,video/mpeg,video/3gpp,video/m3u8,image/gif";
        }
        ...
        async function onFileChange(e) {
          ...
          let data = await _this.uploadOss.upload(fileInput.files[0]).catch((err) => {
            console.log('quillEditor 上传失败', err);
            _this.$message('网络错误，请稍后再试')
            return
