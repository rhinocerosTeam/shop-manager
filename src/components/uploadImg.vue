<template>
  <div :style="{paddingLeft:pl+'px'}">
    ...
    <div class="ivu-form-item-error-tip" v-show="uploadPhotoError && uploadShowError">{{uploadPhoto}}</div>
  </div>
</template>
  ...
  export default{
    ...
    data (){
      return {
        ...
        videoDuration: '',
        expireEndTime: 0,
        uploadPhoto: this.imageType == 2 ? '请上传视频' : '请上传图片',
        uploadSuccessNmae: ''
      }
    },
    components: {},
...
//          console.log(this.filePhoto);
    ...
    computed: {
      uploadPhotoError(){
        ...
        return this.filePhoto.length > 0 ? false : true;
      }
    },
    watch: {
      filePhoto: {
