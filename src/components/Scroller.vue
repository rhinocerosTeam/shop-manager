<template>
    <div class="_v-container" :id="containerId" @touchstart="touchStart($event)" @touchmove="touchMove($event)" @touchend="touchEnd($event)" @mousedown="mouseDown($event)" @mousemove="mouseMove($event)" @mouseup="mouseUp($event)">
        <div class="_v-content" :id="contentId">
            <div v-if="onRefresh" class="pull-to-refresh-layer" :class="{'active': state == 1, 'active refreshing': state == 2,'_v-pull-to-refresh-layer-top':!topHide}">
          ...
          <span class="text" v-if="state != 2">{{ refreshText }}</span>
            ...
            <slot></slot>
            <div v-show='hideLoad' class="loading-layer">
                <span class="spinner-holder" :class="{'active': showLoading}">
          <spinner class="spinner"></spinner>
        </span>
                <div class="no-data-text" :class="{'active': !showLoading && loadingState == 2}" v-text="noDataText">
                </div>
            </div>
        </div>
    </div>
...
<script>
import Scroller from 'plugins/core'
import getContentRender from 'plugins/render';
...
export default {
    ...
    methods: {
        ...
        resetLoadingState() {
            ...
            if (top > this.content.offsetHeight - this.container.clientHeight) {
                setTimeout(() => {
