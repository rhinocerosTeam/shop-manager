...
<template>
  <div class='container' id="waresList" v-loading='loadingFlag'>
    ...
    <el-table :data="dataList" style="width: 100%" ref="waresList" @selection-change="handleSelectionChange">
          ...
          <el-row>
            <el-col :span="18" :offset="3">
              <el-table :data="scope.row.skuList" border style="width: 100%">
                <el-table-column prop="name" align="center" label="规格名称" show-overflow-tooltip>
                </el-table-column>
                <!--<el-table-column prop="id" align="center" label="商品货号">-->
                <!--</el-table-column>-->
                ...
                <el-table-column prop="stockOut" align="center" label="售出数量">
                </el-table-column>
                <el-table-column prop="returnNum" align="center" label="退货数量">
                </el-table-column>
              </el-table>
            </el-col>
          </el-row>
      ...
      <el-table-column align="center" label="规格数量" prop="productId" width="150">
        <template slot-scope="scope">
          {{scope.row.skuList.length}}
        </template>
      </el-table-column>
      <el-table-column align="center" label="库存" prop="stock">
        <template slot-scope="scope">
          {{scope.row.t_stockNum}}
        </template>
      </el-table-column>
      <el-table-column align="center" label="售出数量" prop="stock">
        <template slot-scope="scope">
          {{scope.row.t_stockOut}}
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="操作" width="180">
        <template slot-scope="scope">
          <!--上架状态-->
          <div v-if="scope.row.status == 1">
            <b @click='handleLowerGoodsOk(scope.row,"下架")'>下架</b>
            <b @click='handleSortChange(scope.row)'>更改排序</b>
            <b @click='enterEdit(scope.row)'>编辑</b>
          </div>
          <!--下架状态 或 默认状态-->
          <div v-else>
            <b @click='handleLowerGoodsOk(scope.row,"上架")'>上架</b>
            <b @click='handleSortChange(scope.row)'>更改排序</b>
            <b @click='enterEdit(scope.row)'>编辑</b>
          </div>
        </template>
    ...
    <el-pagination @current-change="handleCurrentChange" :current-page.sync="currentPage" :page-size="pageSize"
                   layout="total, prev, pager, next" :total="totalCount" style="margin: 30px auto">
    </el-pagination>
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        if (res) {
          ...
          if (parseInt(res.code) === 1000) {
            ...
            data.productList.map((obj) => {
              let skuList = obj.skuList
              obj.t_stockNum = 0
              obj.t_stockOut = 0
              obj.t_num = skuList.length; // 总库存

              if (skuList.length > 0) {
                obj.t_stockNum = skuList.reduce((previousValue, currentValue) => {
                  return previousValue.stockNum + currentValue.stockNum
                }).stockNum; // 总库存
                obj.t_stockOut = skuList.reduce((previousValue, currentValue) => {
                  return previousValue.stockOut + currentValue.stockOut
                }).stockOut; // 卖出的库存
      ...
      //下架商品确认 上架 1 下架-1
      handleLowerGoodsOk(param,type, fun) {

        this.$confirm("是否要对已选定商品进行"+type, {
          showClose: false
        }).then(async() => {
