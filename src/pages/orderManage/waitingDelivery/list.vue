    ...
    <div class="order-main">
      ...
      <div class="order-table">
        <template v-for="(items,index) in orderList">
          <!-- 每条订单 组件 -->
          <orderBar @send-delivery='send_delivery' @save-success='getOrderList(pageNo)' :datas='items' :key="index"></orderBar>
        </template>
      </div>
...
export default {
    ...
    methods: {
        ...
        //获得订单列表 数据请求
        getOrderList(pageNo) {
            let type = 2, //0全部 1待支付 2已支付 3已取消 4待收货 5已完成
                { orderNum } = this;
